...
@property (nonatomic,strong) KSYDrawingView *drawView; //画笔视图
...
-(void)addLiveUI{
    ...
    //更换画笔图片
    [liveUIView.floatWindowButton  setBackgroundImage:[UIImage imageNamed:@"画笔"] forState:UIControlStateNormal];
...
-(void)addSubDrawView{
    ...
    _drawView.frame = CGRectMake(60, 80, KSYScreenWidth-120,KSYScreenHeight*0.7);
    [_drawView.layer setBorderColor:[[UIColor redColor] CGColor]];
    [_drawView.layer setBorderWidth:1.0f];
    _wxStreamerKit.drawPicRect = CGRectMake(0.0, 0.15, 1.0, 0.75);
    [self.safeAreaView addSubview:_drawView];
...
-(void)buttonClickAction:(UIButton*)button{
    ...
    //悬浮窗
        ...
        if (button.selected) {
            weakSelf.drawView.hidden = NO;
            weakSelf.wxStreamerKit.drawPic = [[KSYGPUViewCapture alloc] initWithView:self.drawView];
            [weakSelf.wxStreamerKit.drawPic  start];
            
        }
        else{
            [weakSelf.wxStreamerKit.drawPic stop];
            weakSelf.wxStreamerKit.drawPic = nil;
            weakSelf.drawView.hidden = YES;
            [weakSelf.drawView clearAllPath];
        }
    }
    //相机翻转
