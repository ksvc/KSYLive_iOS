...
@property (nonatomic,strong) KSYHeadControl *headControl;
@property (nonatomic,assign) ScreenDirection direction;


@end

@implementation KSYDynamicSwitchVC
...
- (void)viewDidLoad {
    ...
    //    self.safeAreaView.frame = previewRect;
...
-(void)setKitParameter{
    ...
    self.muteState = NO;
    //默认为竖屏方向
    self.direction = ScreenDirectionPortrait;
}
/**
 开始预览
 */
...
-(void)addLiveUI{
    ...
    [liveUIView setUpButtonView:@"横竖屏切换"];
...
- (void)shouldRotateToOrientation:(UIDeviceOrientation)orientation {
    ...
    if (orientation == UIDeviceOrientationLandscapeLeft || orientation == UIDeviceOrientationLandscapeRight) {
        self.direction = ScreenDirectionLandScape;
    ...
    //功能
        ...
        self.collectView.titleBlock = ^(NSString *title,BOOL muteState) {
            if ([title isEqualToString:@"镜像"]) {
                ...
                //                UINavigationController* nav = [[UINavigationController alloc]initWithRootViewController: playUrlQRCodeVc];
...
- (void)setUpLogo:(NSString*)pictureTitle {
    CGFloat yPos ;
    if (self.direction == ScreenDirectionLandScape) {
        yPos = 0.25;
    }
    else {
        yPos = 0.15;
...
- (void) updateAnimateLogo {
    ...
    YYImageFrame* frame = [_animateDecoder frameAtIndex:_animateIdx
                                       decodeForDisplay:NO];
    if (frame.image) {
        _wxStreamerKit.logoPic = [[KSYGPUPicture alloc] initWithImage:frame.image];
    }
