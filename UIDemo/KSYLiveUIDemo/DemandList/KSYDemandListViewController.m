...
@interface KSYDemandListViewController ()
...
@implementation KSYDemandListViewController

- (instancetype)initWithShowType:(VideoListShowType)showType {
    if (self = [super init]) {
        _ksyShowType = showType;
    }
    return self;
}
...
- (void)viewDidLoad {
    [super viewDidLoad];
    if (self.ksyShowType == VideoListShowTypeLive) {
        self.title = @"直播";
    } else if (self.ksyShowType == VideoListShowTypeVod) {
        self.title = @"点播";
    }
    [self setupUI];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (_willAppearFromPlayerView) {
       ...
       self.suspendView.frame = CGRectMake(0, 0, 160, 250);
        ...
        if (self.ksyShowType == VideoListShowTypeVod) {
        ...
        } else if (self.ksyShowType == VideoListShowTypeLive) {
...
- (void)fetchDatasource {
    NSString *urlString = [NSString stringWithFormat:@"https://appdemo.download.ks-cdn.com:8682/api/GetLiveUrl/2017-01-01?Option=%zd", _ksyShowType];
